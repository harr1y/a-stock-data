...
- **就在本仓库的 [`a-stock-data/`](a-stock-data/) 文件夹里**（v3.7.1）。十一层数据架构、54 个端点、19 个数据源，`a-stock-data/SKILL.md` **内嵌全部调用代码**，自包含；除 mootdx 与 baostock 两个 TCP 客户端外，其余均直连 HTTP API。东财接口已内置限流防封，主源被封还能降级到备用源。
...
- **运行依赖**：`pip install mootdx requests pandas stockstats numpy baostock xlrd openpyxl`（自包含，v3.0 起已移除 akshare 依赖）。
...
| 仓库 | 定位 |
|---|---|
| [**a-stock-data**](https://github.com/simonlin1212/a-stock-data) | A 股全栈数据工具包（11 层 · 54 端点 · 19 数据源）—— 本项目的 A 股数据引擎 |
